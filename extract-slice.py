...
vars2ext=raw_input("Which variables to extract? e.g. all or bx by bz etc. ").split()
# Set those variables to be loaded in P3D object
rc.vars2load(vars2ext)
# Ask for time slice to read
slice2ext=int(raw_input("Which slice out of "+str(rc.numslices)+"? "))
# Ask if want to smooth data
smooth=int(raw_input("How much smoothing (integer, 0 for none)? "))
if smooth == '': smooth=0
# Load the time slice
